...
included_data <- load_subcortical_data("../data/processed/processed_tabular.parquet")
...
subcortical_harmonized <- dplyr::left_join(
  harmonized_subcortical_volume, 
  included_data |> dplyr::select(-all_of(brainname_subvolume)),
  by = c('participant_id', 'session_id', 'mr_y_adm__info__dev_serial'))
